...
const exportSelectedProjects = async () => {
  ...
  const enrichedProjects = await Promise.all(
    selectedProjects.map((p) => getFullProjectData(p))
  );

  const json = JSON.stringify(enrichedProjects, null, 2);
  ...
  const projectData = await Promise.all(
    allSnap.docs.map(async (docSnap) => {
      ...
      const taskSnap = await getDocs(
        query(collection(db, "trackerTasks"), where("projectId", "==", docSnap.id))
      );
      const tasks = taskSnap.docs.map((t) => t.data());

      const noteSnap = await getDocs(
        query(collection(db, "trackerNotes"), where("projectId", "==", docSnap.id))
      );
      const notes = noteSnap.docs.map((n) => n.data());
      ...
      const latestTimestamp =
        latestTask?.createdAt?.seconds > latestNote?.createdAt?.seconds
          ? latestTask?.createdAt
          : latestNote?.createdAt;

      return {
        ...project,
        tag: project.tag?.toLowerCase() || null, // normalize tag
  ...
  const filtered = projectData.filter((p) => !p.deleted);
  const trashed = projectData.filter((p) => p.deleted);

  setProjects(filtered);
  setTrashedProjects(trashed);
  setAvailableTags([
    "all",
    ...new Set(filtered.map((p) => p.tag).filter(Boolean))
  ]);
};
...
const filteredProjects =
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white px-4 sm:px-8 py-6 max-w-6xl mx-auto">
      <EditProjectModal
...
/>
      ...
      {/* Tag Filters */}
    <div className="sticky top-0 z-40 bg-gray-900 py-2 mb-4 flex flex-wrap gap-2 border-b border-gray-700">
        ...
        filteredProjects.length === 0 ? (
          ...
          <ul className="space-y-4">
            {filteredProjects.map((proj) => (
              <li
                key={proj.id}
                className="bg-gray-800 rounded-lg p-4 border border-gray-700 shadow hover:shadow-lg transition duration-300"
              >
...
/>
  ...
  <span>Done: {proj.doneTasks}</span>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <span
                      className={`text-xs px-2 py-1 rounded-full text-white ${getColorForTag(
                        proj.tag
                      )}`}
                    >
                      {proj.tag}
                    ...
                    <button
                      onClick={() => handleTrashProject(proj.id)}
...
>
  Edit
                ...
                {/* Expanded Section */}
                {expandedProject === proj.id && (
                  <div className="mt-4 space-y-6 transition-all duration-300 ease-in-out">
                    <div className="border-t border-gray-700 pt-4">
                      <TaskList projectId={proj.id} />
                    </div>
                    <div className="border-t border-gray-700 pt-4">
  ...
  <EditTaskModal task={selectedTask} onClose={() => setSelectedTask(null)} />
        ...
        <ul className="space-y-4">
          {trashedProjects.map((proj) => (
            <li
              key={proj.id}
              className="bg-gray-900 border border-red-500 p-4 rounded shadow"
            >
              <h2 className="text-xl font-semibold text-red-400">{proj.title}</h2>
              <p className="text-sm text-gray-400">{proj.description}</p>
              <div className="mt-2 flex gap-4">
                <button
                  onClick={() =>
                    updateDoc(doc(db, "trackerProjects", proj.id), {
                      deleted: false,
                      deletedAt: null,
                    }).then(fetchProjects)
                  }
                ...
                <button
                  onClick={() =>
                    deleteDoc(doc(db, "trackerProjects", proj.id)).then(
                      fetchProjects
                    )
                  }
